...
async function getMeats(req, res) {
    try {
        const products = await db.query('SELECT * FROM meat');
...
const getMeatbyCategory = async (req, res) => {
    ...
    try {
        ...
        if (products.length === 0) {
            return res.status(404).json({ error: 'No meat item available in this category' });
        }

        // Assuming there's a directory set up for meat item images
        ...
        console.error('Error fetching meat by category:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
};
...
async function getMeatsById(req, res) {
...
async function updateMeat(req, res) {
    ...
    try {
        ...
        await db.query(updateQuery, updateValues);
         ...
         const updatedProduct = await db.query('SELECT * FROM meat WHERE prod_id = ?', [id]);
        // Transform the image path to full URLs
        const transformedResults = {
            ...updatedProduct[0],
        ...
        // Return success message
        res.status(200).json({ message: 'meat updated successfully',transformedResults });
    } catch (error) {
...
module.exports ={createMeat,upload,getMeats,getMeatsById,deleteMeat,updateMeat,getMeatbyCategory};
