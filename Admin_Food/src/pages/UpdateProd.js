  ...
  useEffect(() => {
    ...
    const fetchProductDetails = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:3001/api/product/?id=${productId}`);
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
    ...
    <>
      ...
      <form className="row g-3 events-details" onSubmit={handleSave}>
        ...
        <div className="col-md-6">
          ...
          <input type="file" className="form-control" id="image" name="image" onChange={(e) => handleImageChange(e, formData.image)} />
          <input type="file" className="form-control" id="image" name="image" onChange={(e) => handleImageChange(e, formData.image)} />
          <input type="file" className="form-control" id="image" name="image" onChange={(e) => handleImageChange(e, formData.image)} />
        </div>
