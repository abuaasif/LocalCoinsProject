  ...
  useEffect(() => {
    ...
    const fetchProductDetails = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:3001/api/getmeatById?id=${productId}`);
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
  ...
  const handleSave = async (e) => {
    ...
    try {
      // const response = await fetch(`http://localhost:3001/api/updateProduct/${product_id}`, {
        const response = await fetch(`http://localhost:3001/api/updatemeats?id=${productId}`, {
