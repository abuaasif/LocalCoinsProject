...
const createProduct = async (req, res) => {
    ...
    if (req.files && req.files.length > 0) {
        imageFilenames = req.files.map(file => `${req.protocol}://${req.get('host')}/uploadPath/${file.filename}`);
    }
    // Process image URLs
    ...
    imagesInput.forEach((imageInput, index) => {
        ...
        let filename = `Image_${timestamp}_${req.body.name}_${index}_.${fileExtension}`;
