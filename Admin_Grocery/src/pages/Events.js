...
export const EventsOne = () => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const response = await fetch('http://localhost:3001/api/catgrocery');
        if (!response.ok) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:3001/api/creategrocery', {
        method: 'POST',
    ...
    <>
      ...
      <form className="row g-3 events-details" onSubmit={handleSubmit}>
        ...
        <div className="col-md-6">
          ...
          <input type="file" className="form-control" id="image" name="images" onChange={handleImageChange} />
          <input type="file" className="form-control" id="image" name="images" onChange={handleImageChange} />
          <input type="file" className="form-control" id="image" name="images" onChange={handleImageChange} />
          <input type="file" className="form-control" id="image" name="images" onChange={handleImageChange} />
        </div>
