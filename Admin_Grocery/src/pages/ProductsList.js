...
const ProductsList = () => {
  ...
  const fetchProducts = async () => {
    try {
      const response = await fetch('http://localhost:3001/api/getAllGrocerys');
  ...
  const deleteProduct = async (productIds) => {
    try {
      await fetch(`http://localhost:3001/api/deletegrocery?id=${productIds}`, {
        method: 'DELETE',
        headers: {
  ...
  const editProduct = async (product) => {
    ...
    navigate('/Updategrocery',{state:{product_id:product.prod_id}})
